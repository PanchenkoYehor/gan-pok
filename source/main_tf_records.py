...
def dataset_from_image_files(input_file_pattern: str):
    # tf.io.decode_image returns a tensor with no shape when fed a JPEG input. So we have to use
    # tf.io.decode_jpeg instead in that case.
    decode_fn = (tf.io.decode_jpeg
        if input_file_pattern.endswith('jpg') or input_file_pattern.endswith('jpeg') else
        tf.io.decode_image)
    def prepare(file_name: os.PathLike):
        return decode_fn(tf.io.read_file(file_name))
